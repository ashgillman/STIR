...
ArcCorrectionTests::run_tests_for_specific_proj_data_info(const ArcCorrection& arc_correction) {
  ...
  for (int segment_num = proj_data_info_noarc_corr.get_min_segment_num();
    ...
    Sinogram<float> noarccorr_sinogram = proj_data_info_noarc_corr.get_empty_sinogram(axial_pos_num, segment_num);
    Sinogram<float> arccorr_sinogram = proj_data_info_arc_corr.get_empty_sinogram(axial_pos_num, segment_num);

    for (int view_num = proj_data_info_noarc_corr.get_min_view_num(); view_num <= proj_data_info_noarc_corr.get_max_view_num();
         view_num += 3) {
      Viewgram<float> noarccorr_viewgram = proj_data_info_noarc_corr.get_empty_viewgram(view_num, segment_num);
      Viewgram<float> arccorr_viewgram = proj_data_info_arc_corr.get_empty_viewgram(view_num, segment_num);
      // test geometry by checking if single non-zero value gets put in the right bin
      {
        for (int tangential_pos_num = proj_data_info_noarc_corr.get_min_tangential_pos_num();
          ...
          const float noarccorr_s =
              proj_data_info_noarc_corr.get_s(Bin(segment_num, view_num, axial_pos_num, tangential_pos_num));
          const float arccorr_s =
              proj_data_info_arc_corr.get_s(Bin(segment_num, view_num, axial_pos_num, arccorr_tangential_pos_num_at_max));
