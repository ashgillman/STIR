...
detail::ParallelprojHelper::ParallelprojHelper(const ProjDataInfo& p_info, const DiscretisedDensity<3, float>& density)
    : xstart(p_info.size_all() * 3), xend(p_info.size_all() * 3) {
  ...
  for (int seg : segment_sequence) {
    bin.segment_num() = seg;
    for (bin.axial_pos_num() = p_info.get_min_axial_pos_num(bin.segment_num());
         bin.axial_pos_num() <= p_info.get_max_axial_pos_num(bin.segment_num()); ++bin.axial_pos_num()) {
      for (bin.view_num() = p_info.get_min_view_num(); bin.view_num() <= p_info.get_max_view_num(); ++bin.view_num()) {
        for (bin.tangential_pos_num() = p_info.get_min_tangential_pos_num();
             bin.tangential_pos_num() <= p_info.get_max_tangential_pos_num(); ++bin.tangential_pos_num()) {
